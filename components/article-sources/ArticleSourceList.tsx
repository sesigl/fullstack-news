...
export default function ArticleSourceList(props: { articleSources: ArticleSourceOverviewViewModel[] }) {

  return (
    <div className="max-w-xl px-5 mx-auto sm:px-8 lg:px-0 md:max-w-2xl lg:max-w-none lg:pr-48">
      <div className="relative">
        <h3 className="pb-2.5 text-2xl font-medium text-gray-900 border-b border-gray-300/70 relative before:content-[''] before:left-0 before:w-24 before:h-px before:-bottom-px before:bg-red-600 before:absolute">RSS Feeds</h3>
        <Link href="/article-source/create">
      ...
      <div className="relative mt-10">
        <div className="grid gap-12 sm:grid-cols-1">
          ...
          {props.articleSources.map(as => {

            let lastParsedStr = ""
            if (as.lastArticleParsedDate) {
              let date = new Date(as.lastArticleParsedDate);
              let now = new Date()
              let millisFromNow = now.getTime() - date.getTime()
              lastParsedStr = "(Last article parsed " + (millisFromNow < 1000 ? 'Now' : hdate.relativeTime(date)) + ")"
            }


            return (
              <div key={as.id}>
                <Link href={ `/article-sources/${as.id}`}>
                  <h3 className="text-lg font-medium leading-6 text-gray-900 ">{as.rssUrl}</h3>
                </Link>
                <div className="mt-2 space-y-1 text-md">
                  <p className="text-gray-500">
                    {as.articleCount} articles parsed {lastParsedStr}
                  </p>
                  {
                    as.approved ?
