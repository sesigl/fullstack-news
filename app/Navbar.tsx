...
import capitalizeFirstLetter from "../libs/parser/infrastructure/js/string/capitalizeFirstLetter";
import SortMode, {getIconForSortMode} from "../libs/interfaces/SortMode";
import AlgoliaSearch from "../components/layout/AlgoliaSearch";
import UserNavItemsDesktop from "../components/layout/user/UserNavItemsDesktop";
import UserSubMenuMobile from "../components/layout/user/UserSubMenuMobile";
...
export default function Navbar() {
  ...
  const sortByValue = searchParams.get('sort') ? capitalizeFirstLetter(searchParams.get('sort') as string) : 'Hot'
  return (
                ...
                <div className="flex items-center shrink-0">
                  ...
                  </Link>
                  {
                      ...
                      <div className="md:hidden ml-6 flex flex-col items-center"
                           title={"Sorted by " + sortByValue}>{searchParams.get('sort') ?
                          <span>{getIconForSortMode(searchParams.get('sort') as SortMode)}</span> : getIconForSortMode(SortMode.HOT)} {sortByValue}</div>
                  }
                </div>
...

                  {
                    mainItems.map(item => (
                        (<Link
                          ...
                          className={`flex flex-col content-center justify-items-center px-3 py-1 font-medium text-md ${(pathname === "/" || pathname === "/sort/[sort]") && searchParams.get('sort') === item.activeSortValue ? 'text-red-700' : 'text-gray-800 transition duration-300 ease-in-out hover:text-red-700'}`}>
                  ...
                  <div className="pl-2 lg:pl-10 flex flex-row flex-shrink-0">
                            {pagesSubmenu.map(item => (
                                (<Link
                                  ...
                                  className={`flex flex-col content-center justify-items-center px-3 py-1 font-medium text-md transition duration-300 ease-in-out hover:text-red-700 ${(pathname === item.link) ? 'text-red-700' : ''}`}>
                    ...
                    <div className="px-2 pt-2 pb-3 space-y-1">
                      {
                        mainItems.map(item => (
                            (<Link
                              ...
                              className={`flex flex-row content-start justify-items-start block px-4 py-3 font-medium rounded-lg ${(pathname === "/" || pathname === "/sort/[sort]") && searchParams.get('sort') === item.activeSortValue ? 'bg-gray-50 text-red-700' : 'text-gray-800 hover:bg-gray-50 hover:text-red-700 transition duration-300 ease-in-out'}`}
                              aria-current="page">
                        ...
                        <Fragment>
                          {pagesSubmenu.map((subLink, j) => (
                              (<Link
                                href={subLink.link}
                                key={j}
                                className={`block px-4 py-2 font-medium rounded-lg ${pathname == subLink.link ? 'bg-gray-50 text-red-700' : 'text-gray-600 hover:bg-gray-50 hover:text-red-700 transition duration-300 ease-in-out'}`}
                                aria-current="page">
